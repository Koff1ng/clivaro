  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-blue-50/30 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
            ...
            <SpotlightCard className="relative border-2 border-gray-200/50 dark:border-gray-700/50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm" spotlightColor="rgba(59, 130, 246, 0.15)">
              <CardContent className="p-0">
                  ...
                  <div className="md:hidden space-y-4 p-4">
                    {comparison.map((item, index) => (
                      <div key={index} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                        <h4 className="font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">{item.feature}</h4>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Otras Soluciones</p>
                            <p className="text-sm text-slate-600 dark:text-slate-400">{item.competencia}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-[10px] uppercase font-bold text-blue-500 mb-1">Clivaro</p>
                            <div className="flex items-center justify-end gap-2">
                              {item.winner === 'clivaro' && <Check className="w-4 h-4 text-green-500" />}
                              <p className="text-sm font-bold text-blue-600 dark:text-blue-400">{item.clivaro}</p>
                            </div>
                          </div>
                        </div>
                      </div>
      ...
      <a
        ...
        target="_blank"
        className="fixed bottom-8 right-8 z-[100] bg-[#25D366] text-white p-4 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group hover:animate-none"
