...
-- -----------------------------------------------------------------------------
-- RESTAURANT MODE & NEW SCHEMA UPDATES (ROBUST VERSION)
-- -----------------------------------------------------------------------------

-- 0. Ensure Base Tables Exist (For existing tenants that might be very old or broken)
...
-- 1. Updates to existing tables (Idempotent Column Additions)

-- Product updates
DO $$
BEGIN
    BEGIN
        ALTER TABLE "Product" ADD COLUMN "productType" TEXT NOT NULL DEFAULT 'RETAIL';
    EXCEPTION WHEN duplicate_column THEN NULL; END;

    BEGIN
        ALTER TABLE "Product" ADD COLUMN "enableRecipeConsumption" BOOLEAN NOT NULL DEFAULT false;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
END $$;

-- StockMovement updates
DO $$
BEGIN
    BEGIN
    ...
    BEGIN
        ALTER TABLE "StockMovement" ADD COLUMN "reasonNote" TEXT;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
END $$;

-- TenantSettings updates
DO $$
BEGIN
    BEGIN
        ALTER TABLE "TenantSettings" ADD COLUMN "enableRestaurantMode" BOOLEAN DEFAULT false;
    EXCEPTION WHEN duplicate_column THEN NULL; END;

    -- Ensure other key fields exist too in case table existed but was old
    BEGIN
        ALTER TABLE "TenantSettings" ADD COLUMN "customSettings" TEXT;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
    
    BEGIN
        ALTER TABLE "TenantSettings" ADD COLUMN "onboardingCompleted" BOOLEAN DEFAULT false;
    EXCEPTION WHEN duplicate_column THEN NULL; END;
END $$;

-- 2. New Tables (Unit, UnitConversion, Recipe, RecipeItem)
