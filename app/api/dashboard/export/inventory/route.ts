...
export async function GET(request: Request) {
    ...
    try {
        ...
        const result = await withTenantRead(tenantId, async (prisma) => {
            const [products, stockLevels] = await Promise.all([
                prisma.product.findMany({
                    where: { active: true },
                    select: { id: true, name: true, sku: true, category: true, cost: true }
                }),
        ...
        result.stockLevels.forEach(sl => {
            if (sl.productId) stockByProduct.set(sl.productId, (stockByProduct.get(sl.productId) || 0) + Number(sl.quantity || 0))
        })
        ...
        logger.error('Error exporting inventory to Excel', error)
